...
wallet file");
 // we'll establish a connection to Solana devnet using the const wedefined above
...
sig);
 ...
 fn transfer_sol() {
    ...
    // Here i think we need to provide the message bytes instead hash of signature.
    match sig.verify(&pubkey.to_bytes(), message_bytes) {
    true => println!("Signature verified"),
    false => println!("Verification failed"),
    }

    let to_pubkey = Pubkey::from_str("6VNFsAtbXaELkgQHqmD2WYSTK5JByiV1GZgYmDcUoNfb").unwrap();
 ...
 fn submit_code() {
    ...
    let signer = read_keypair_file("Turbin3-wallet.json").expect("Couldn't find
    wallet file");
    ...
    let (authority, _bump) = Pubkey::find_program_address(seeds,
    &turbin3_prereq_program);
